...
SequenceTextFile::SequenceTextFile(const QString &filename) {
...
bool SequenceTextFile::parseSectionSettings(const QStringList &sectionList,
    ...
    for (i = 0; i < sectionList.size(); ++i) {
        ...
        if (key == KEY_NAME_PROJECT_NAME) {
            ...
            settings.voiceDirectory = fromUSTVoiceDir(value, Sys::appPath()); // Voice Directory
        } else if (key == KEY_NAME_CACHE_DIR) {
            settings.cacheDirectory = value; // Cache Directory
        } else if (key == KEY_NAME_TOOL1) {
            settings.wavtoolPath = fromUSTToolsDir(value, Sys::appPath()); // Wavtool
        } else if (key == KEY_NAME_TOOL2) {
            settings.resamplerPath = fromUSTToolsDir(value, Sys::appPath()); // Resampler
...
void SequenceTextFile::writeSectionSettings(QTextStream &oStream) {
    ...
    oStream << KEY_NAME_VOICE_DIR << "="
            << toUSTVoiceDir(sectionSettings.voiceDirectory, Sys::appPath()) << Qt::endl;
    ...
    oStream << KEY_NAME_TOOL1 << "=" << toUSTToolsDir(sectionSettings.wavtoolPath, Sys::appPath())
            << Qt::endl;
    oStream << KEY_NAME_TOOL2 << "=" << toUSTToolsDir(sectionSettings.resamplerPath, Sys::appPath())
            << Qt::endl;
