...
double UtaPitchCurves::f_type(const Utau::PointType &ptype, const double &x1, const double &y1,
...
QList<int> UtaPitchCurves::convert_from_vector_point(
    ...
    const QList<QControlPoint> &nextNote, const QList<double> &nextVBR, const double &nextPre,
    const double &nextOve, const int &nextLength, const QList<QControlPoint> &prevNote,
...
int UtaPitchCurves::find_impact(const QList<QControlPoint> &portamento, int &startIndex,
                                int curTick, const double &PositiveTempo, double NegativeTempo,
    ...
    if (!portamento.empty()) {
        ...
        if (tooLeft) {
            basePitch = int(portamento[0].mY * 10); // haven't reached the first point yet
        }
    }
