...
QList<RenderArgs> QBatchRenderer::getArgs(QPoint range) const {
    ...
    for (int i = left; i <= right; ++i) {
        ...
        QString aFlags = prop(aNote.flags, Utau::Flags);

        double aTempo = prop(aNote.tempo, Utau::Tempo);

        double aIntensity = prop(aNote.intensity, Utau::Intensity);
        double aModulation = prop(aNote.modulation, Utau::Modulation);
        double aVelocity = prop(aNote.velocity, Utau::Velocity);

        QGenonSettings aGenon = notes.at(i).g;
        QCorrectGenon aCorrect = notes.at(i).c;
        QString aFilename;
        ...
        if (aGenon.mFileName.isEmpty()) {
            ...
            aFilename = voice + SLASH + aGenon.mFileName;
        }
        ...
        if (i < notes.size() - 1) {
            ...
            QCorrectGenon nextGenon = notes.at(i + 1).c;
        ...
        // Real Length
        double aDuration = Utau::TickToTime(aLength, aTempo); // convert from ticks
