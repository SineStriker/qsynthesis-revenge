...
public:
    AudioFileImpl(AudioError error, AVFormatContext *format_context, AVCodecContext *codec_context,
    ...
    AudioError get_error() const override {
        return this->error;
    }
    std::string get_codec_name() const override {
        return this->codec_name;
    }
    int get_bit_rate() const override {
    ...
    int get_streams() const override {
        return this->streams;
    }
    ...
    const float *get_buffer() const override {
        return this->buffer;
    }
    int64_t get_frames_per_interval() const override {
        return this->frames_per_interval;
    }
    int64_t get_error_per_interval() const override {
        return this->error_per_interval;
    }
    int64_t get_error_base() const override {
...
std::unique_ptr<AudioFile> Audio::open(const std::string &file_name, int stream) {
    ...
    if (!error) {
        ...
        if (codecpar->codec_id == AV_CODEC_ID_AAC || codecpar->codec_id == AV_CODEC_ID_MUSEPACK8 ||
            codecpar->codec_id == AV_CODEC_ID_WMAV1 || codecpar->codec_id == AV_CODEC_ID_WMAV2) {
    ...
    if (!error) {
        ...
        if (fmt != AV_SAMPLE_FMT_S16 && fmt != AV_SAMPLE_FMT_S16P && fmt != AV_SAMPLE_FMT_S32 &&
            fmt != AV_SAMPLE_FMT_S32P && fmt != AV_SAMPLE_FMT_FLT && fmt != AV_SAMPLE_FMT_FLTP &&
            fmt != AV_SAMPLE_FMT_DBL && fmt != AV_SAMPLE_FMT_DBLP) {
            error = AudioError::BAD_SAMPLE_FORMAT;
        }
    }

    return std::unique_ptr<AudioFile>(
        new AudioFileImpl(error, format_context, codec_context, audio_stream, codec_name, bit_rate,
                          sample_rate, bits_per_sample, streams, channels, duration));
}

AudioFileImpl::AudioFileImpl(AudioError error, AVFormatContext *format_context,
                             AVCodecContext *codec_context, int audio_stream,
                             const std::string &codec_name, int bit_rate, int sample_rate,
                             int bits_per_sample, int streams, int channels, double duration)
    : error(error), format_context(format_context), codec_context(codec_context),
      ...
      sample_rate(sample_rate), bits_per_sample(bits_per_sample), streams(streams),
...
int AudioFileImpl::read() {
    ...
    int samples = (int) pcm_data.size();
    if (samples > this->buffer_len) {
        this->buffer = static_cast<float *>(av_realloc(this->buffer, samples * sizeof(float)));
        this->buffer_len = samples;
    }
    int i = 0;
    for (auto it = pcm_data.begin(); it != pcm_data.end(); ++it) {
        buffer[i++] = *it;
    }
