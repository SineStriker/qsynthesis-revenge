...
SpekRuler::SpekRuler(int x, int y, Position pos, QString sample_label, int *factors, int min_units,
                     int max_units, double spacing, double scale, double offset,
                     formatter_cb formatter)
    : x(x), y(y), pos(pos), sample_label(sample_label), factors(factors), min_units(min_units),
      max_units(max_units), spacing(spacing), scale(scale), offset(offset), formatter(formatter) {
}

void SpekRuler::draw(QPainter &dc) {
    QFontMetrics f(QApplication::font());
    int w = f.horizontalAdvance(sample_label);
    int h = f.height();
...
void SpekRuler::draw_tick(QPainter &dc, int tick) {
    ...
    int value =
        this->pos == TOP || this->pos == BOTTOM ? tick : this->max_units + this->min_units - tick;
