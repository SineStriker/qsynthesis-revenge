...
QTextCodec *QMCodec::GetUtfCodec(const QByteArray &data, bool *determined) {
...
QString QMCodec::unescape(const QString &s) {
    QString res;
    auto p = s.data();
    while (!p->isNull()) {
        if (*p == '\\') {
...
QByteArray QMCodec::toNativePath(const QByteArray &s) {
    QByteArray s2 = s;
#ifdef Q_OS_WINDOWS
    return s2.replace('/', '\\');
#else
...
QByteArray QMCodec::fromNativePath(const QByteArray &s) {
    QByteArray s2 = s;
    return s2.replace('\\', '/');
}