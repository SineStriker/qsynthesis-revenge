...
ConfigWidget::ConfigWidget(QWidget *parent) : QWidget(parent) {
    ...
    m_model->loadJsonFile(":/res/config.json");

    QHBoxLayout *buttonLayout = new QHBoxLayout;
    buttonLayout->addStretch();
    QPushButton *saveButton = new QPushButton(tr("Save"), this);
    connect(saveButton, &QPushButton::clicked, this, &ConfigWidget::saveJson);
...
void ConfigWidget::saveJson() {
    QJsonObject jsonObject = saveTreeViewToJson(m_model);
    QFile file("D:\\projects\\qsynthesis-revenge\\src\\tests\\LyricTest\\out.json");
