...
public:
    inline MyCommandLineParserPrivate()
        : singleDashWordOptionMode(MyCommandLineParser::ParseAsCompactedShortOptions),
          optionsAfterPositionalArgumentsMode(MyCommandLineParser::ParseAsOptions),
    ...
    MyCommandLineParser::SingleDashWordOptionMode singleDashWordOptionMode;

    //! How to parse "arg -option"
    MyCommandLineParser::OptionsAfterPositionalArgumentsMode optionsAfterPositionalArgumentsMode;
...
Q_DECLARE_TYPEINFO(MyCommandLineParserPrivate::PositionalArgumentDefinition, Q_MOVABLE_TYPE);

QStringList MyCommandLineParserPrivate::aliases(const QString &optionName) const {
    const NameHash_t::const_iterator it = nameHash.constFind(optionName);
    if (it == nameHash.cend()) {
        qWarning("MyCommandLineParser: option not defined: \"%ls\"", qUtf16Printable(optionName));
        return QStringList();
    }
...
/*!
    ...
    \brief The MyCommandLineParser class provides a means for handling the
    command line options.

    QCoreApplication provides the command-line arguments as a simple list of strings.
    MyCommandLineParser provides the ability to define a set of options, parse the
    ...
    Example:
    \snippet code/src_corelib_tools_MyCommandLineParser_main.cpp 0

    If your compiler supports the C++11 standard, the three addOption() calls in
    the above example can be simplified:
    \snippet code/src_corelib_tools_MyCommandLineParser_main.cpp cxx11

    Known limitation: the parsing of Qt options inside QCoreApplication and subclasses
    happens before MyCommandLineParser exists, so it can't take it into account. This
    ...
    QCoreApplication::arguments() before MyCommandLineParser defines the \c{profile}
    option and parses the command line.

    \section2 How to Use MyCommandLineParser in Complex Applications
    ...
    On Windows, MyCommandLineParser uses message boxes to display usage information
...
*/
MyCommandLineParser::MyCommandLineParser() : d(new MyCommandLineParserPrivate) {
}

/*!
    Destroys the command line parser object.
*/
MyCommandLineParser::~MyCommandLineParser() {
    delete d;
}

/*!
    \enum MyCommandLineParser::SingleDashWordOptionMode
...
*/
void MyCommandLineParser::setSingleDashWordOptionMode(
    MyCommandLineParser::SingleDashWordOptionMode singleDashWordOptionMode) {
    d->singleDashWordOptionMode = singleDashWordOptionMode;
}

/*!
    \enum MyCommandLineParser::OptionsAfterPositionalArgumentsMode
...
/*!
    ...
    \since 5.6
*/
void MyCommandLineParser::setOptionsAfterPositionalArgumentsMode(
    MyCommandLineParser::OptionsAfterPositionalArgumentsMode parsingMode) {
 ...
 */
bool MyCommandLineParser::addOption(const QCommandLineOption &option) {
    const QStringList optionNames = option.names();

    if (!optionNames.isEmpty()) {
        for (const QString &name : optionNames) {
            if (d->nameHash.contains(name)) {
                qWarning() << "MyCommandLineParser: already having an option named" << name;
                return false;
            }
        }
...
*/
bool MyCommandLineParser::addOptions(const QList<QCommandLineOption> &options) {
    // should be optimized (but it's no worse than what was possible before)
    bool result = true;
    for (QList<QCommandLineOption>::const_iterator it = options.begin(), end = options.end();
         it != end; ++it)
        result &= addOption(*it);
    return result;
}
...
/*!
    ...
    This option is handled automatically by MyCommandLineParser.
...
*/
QCommandLineOption MyCommandLineParser::addVersionOption() {
    QCommandLineOption opt(QStringList() << QStringLiteral("v") << QStringLiteral("version"),
...
/*!
    ...
    These options are handled automatically by MyCommandLineParser.
    ...
    Returns the option instance, which can be used to call isSet().
*/
QCommandLineOption MyCommandLineParser::addHelpOption() {
...
*/
void MyCommandLineParser::setApplicationDescription(const QString &description) {
...
*/
QString MyCommandLineParser::applicationDescription() const {
...
/*!
    ...
    Example:
    \snippet code/src_corelib_tools_MyCommandLineParser.cpp 2

    \sa addHelpOption(), helpText()
*/
void MyCommandLineParser::addPositionalArgument(const QString &name, const QString &description,
                                                const QString &syntax) {
    MyCommandLineParserPrivate::PositionalArgumentDefinition arg;
    arg.name = name;
...
/*!
    ...
    Example:
    \snippet code/src_corelib_tools_MyCommandLineParser.cpp 3
*/
void MyCommandLineParser::clearPositionalArguments() {
...
/*!
    ...
    Returns \c false in case of a parse error (unknown option or missing value); returns \c true
   otherwise.

    \sa process()
*/
bool MyCommandLineParser::parse(const QStringList &arguments) {
...
*/
QString MyCommandLineParser::errorText() const {
    if (!d->errorText.isEmpty())
        return d->errorText;
    if (d->unknownOptionNames.count() == 1)
        return MyCommandLineParser::tr("Unknown option '%1'.").arg(d->unknownOptionNames.first());
    if (d->unknownOptionNames.count() > 1)
        return MyCommandLineParser::tr("Unknown options: %1.")
            .arg(d->unknownOptionNames.join(QStringLiteral(", ")));
    return QString();
}
 ...
 */
void MyCommandLineParser::process(const QStringList &arguments) {
    if (!d->parse(arguments)) {
        showParserMessage(QCoreApplication::applicationName() + QLatin1String(": ") + errorText() +
                              QLatin1Char('\n'),
                          ErrorMessage);
 ...
 */
void MyCommandLineParser::process(const QCoreApplication &app) {
    // QCoreApplication::arguments() is static, but the app instance must exist so we require it as
    // parameter
    Q_UNUSED(app);
    process(QCoreApplication::arguments());
}

void MyCommandLineParserPrivate::checkParsed(const char *method) {
    if (needsParsing)
        qWarning("MyCommandLineParser: call process() or parse() before %s", method);
}
 ...
 */
bool MyCommandLineParserPrivate::registerFoundOption(const QString &optionName) {
...
/*!
    ...
    \param argument the argument from the command line currently parsed. Only used for -k=value
   parsing. \param argumentIterator iterator to the currently parsed argument. Incremented if the
   next argument contains the value. \param argsEnd args.end(), to check if ++argumentIterator goes
   out of bounds Returns \c true on success.
 */
bool MyCommandLineParserPrivate::parseOptionValue(const QString &optionName,
        ...
        if (withValue) {
            if (assignPos == -1) {
                ...
                if (*argumentIterator == argsEnd) {
                    errorText = MyCommandLineParser::tr("Missing value after '%1'.").arg(argument);
                    return false;
                }
            ...
            if (assignPos != -1) {
                errorText = MyCommandLineParser::tr("Unexpected value after '%1'.")
                                .arg(argument.left(assignPos));
                return false;
            }
        }
    }
 ...
 */
bool MyCommandLineParserPrivate::parse(const QStringList &args) {
    needsParsing = false;
    bool error = false;
    ...
    if (args.isEmpty()) {
        qWarning("MyCommandLineParser: argument list cannot be empty, it should contain at least "
                 "the executable name");
        return false;
    }
    ...
    for (; argumentIterator != args.end(); ++argumentIterator) {
        ...
        if (forcePositional) {
            ...
            switch (singleDashWordOptionMode) {
                case MyCommandLineParser::ParseAsCompactedShortOptions: {
                    QString optionName;
                    ...
                    for (int pos = 1; pos < argument.size(); ++pos) {
                        ...
                        if (!registerFoundOption(optionName)) {
                            ...
                            const NameHash_t::const_iterator nameHashIt =
                                nameHash.constFind(optionName);
                            Q_ASSERT(nameHashIt !=
                                     nameHash.constEnd()); // checked by registerFoundOption
                            const NameHash_t::mapped_type optionOffset = *nameHashIt;
                            const bool withValue =
                                !commandLineOptionList.at(optionOffset).valueName().isEmpty();
                            if (withValue) {
                    ...
                    if (!valueFound &&
                        !parseOptionValue(optionName, argument, &argumentIterator, args.end()))
                        error = true;
                    break;
                }
                case MyCommandLineParser::ParseAsLongOptions: {
                    if (argument.size() > 2) {
            ...
            if (optionsAfterPositionalArgumentsMode ==
                MyCommandLineParser::ParseAsPositionalArguments)
                forcePositional = true;
        }
...
/*!
    ...
    Example:
    \snippet code/src_corelib_tools_MyCommandLineParser.cpp 0
 */

bool MyCommandLineParser::isSet(const QString &name) const {
    d->checkParsed("isSet");
...
QString MyCommandLineParser::value(const QString &optionName) const {
    d->checkParsed("value");
...
QStringList MyCommandLineParser::values(const QString &optionName) const {
...
/*!
    ...
    Example:
    \snippet code/src_corelib_tools_MyCommandLineParser.cpp 1
*/
bool MyCommandLineParser::isSet(const QCommandLineOption &option) const {
...
/*!
    ...
    \sa values(), QCommandLineOption::setDefaultValue(), QCommandLineOption::setDefaultValues()
*/
QString MyCommandLineParser::value(const QCommandLineOption &option) const {
...
/*!
    ...
    \sa value(), QCommandLineOption::setDefaultValue(), QCommandLineOption::setDefaultValues()
*/
QStringList MyCommandLineParser::values(const QCommandLineOption &option) const {
...
QStringList MyCommandLineParser::positionalArguments() const {
...
QStringList MyCommandLineParser::optionNames() const {
...
QStringList MyCommandLineParser::unknownOptionNames() const {
...
/*!
    ...
    \since 5.4
*/
Q_NORETURN void MyCommandLineParser::showVersion() {
...
/*!
    ...
    \sa helpText()
*/
Q_NORETURN void MyCommandLineParser::showHelp(int exitCode) {
    d->showHelp(exitCode, false);
}

Q_NORETURN void MyCommandLineParserPrivate::showHelp(int exitCode, bool includeQtOptions) {
...
/*!
    ...
    \sa showHelp()
*/
QString MyCommandLineParser::helpText() const {
...
static QString wrapText(const QString &names, int optionNameMaxWidth, const QString &description) {
    ...
    for (int i = 0; i < len; ++i) {
        ...
        if (breakAt != -1) {
            ...
            // qDebug() << "breakAt=" << description.at(breakAt) << "breakAtSpace=" << breakAtSpace
            // << lineStart << "to" << breakAt << description.mid(lineStart, numChars);
            text += indentation + nextNameSection().leftJustified(optionNameMaxWidth) +
                    QLatin1Char(' ');
...
QString MyCommandLineParserPrivate::helpText(bool includeQtOptions) const {
    const QLatin1Char nl('\n');
    QString text;
    QString usage;
    ...
    if (includeQtOptions) {
        // addQtOptions(&options);
    }
    if (!options.isEmpty())
        usage += QLatin1Char(' ') + MyCommandLineParser::tr("[options]");
    for (const PositionalArgumentDefinition &arg : positionalArgumentDefinitions)
        usage += QLatin1Char(' ') + arg.syntax;
    text += MyCommandLineParser::tr("Usage: %1").arg(usage) + nl;
    if (!description.isEmpty())
        text += description + nl;
    text += nl;
    if (!options.isEmpty())
        text += MyCommandLineParser::tr("Options:") + nl;
    ...
    if (!positionalArgumentDefinitions.isEmpty()) {
        if (!options.isEmpty())
            text += nl;
        text += MyCommandLineParser::tr("Arguments:") + nl;
        for (const PositionalArgumentDefinition &arg : positionalArgumentDefinitions)
