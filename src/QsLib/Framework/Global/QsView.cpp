...
QPixmap QsView::createBitmapFromSVG(QString fullpath, QSize size) {
 ...
 */
QPixmap QsView::createDeviceRenderPixmap(QWindow *refWindow, QSize logicalPixelSize) {
#ifdef Q_OS_MACOS
    qreal targetDPR;
    if (refWindow)
...
bool QsView::widgetHitTest(QWidget *w1, QWidget *w2) {
...
bool QsView::rectHitTest(const QRect &rect1, const QRect &rect2) {
    return rectHitTest(QRectF(rect1), QRectF(rect2));
}

bool QsView::rectHitTest(const QRectF &rect1, const QRectF &rect2) {
...
void QsView::waitToShow(QWidget *w) {
    while (!w->isVisible()) {
        qApp->processEvents(QEventLoop::AllEvents);
    }
}

void QsView::centralizeWindow(QWidget *w, QSizeF ratio) {
    QSize desktopSize;
    if (w->parentWidget()) {
...
void QsView::enableScaling() {
    qputenv("QT_ENABLE_HIGHDPI_SCALING", "1");
    QGuiApplication::setHighDpiScaleFactorRoundingPolicy(
        Qt::HighDpiScaleFactorRoundingPolicy::PassThrough);
}

QPoint QsView::fixDesktopPos(const QPoint &pos, const QSize &size) {
    QPoint target(pos);
    if (target.x() < 0) {
        target.setX(0);
    }
...
void QsView::bringWindowToForeground(QWidget *w) {
