...
+ 构建的对象一般只有三种，lib(a)、dll(so)与exe，即静态库、动态库与可执行文件，将它们统称为目标。
...
2. 预构建的第三方库
    + 包括FFmpeg、SDL、zlib等...
    + 你需要在拉取本仓库后执行`setup_vcpkg.py`对这些第三方库进行初始化，它们会被安装到项目目录的`vcpkg/installed/{TRIPLET}/lib`中，通过vcpkg提供的工具链脚本被我们的项目使用。

### 基础内部库

本工程也有大量的内部库，这些内部库以源码形式存在于本仓库中，需要参与编译构建。这些内部库中，也有基础与上层之分，基础的内部库有两种。

1. 实用库
    + 这些库要么是自己编写的工具库，要么是源码形式的第三方库
...
本工具集有多个应用程序，每个程序拥有专门属于自己的几个动态库，些库就是程序内部库，不同程序的程序内部库不应当有大量交集，因为这些交集可以提取到公共内部库中。
...
+ WinMain
    ...
    + 这时又有一个问题，我们在编译S库的时候，必然链接了其他动态库，那么Windows同样地，必须先搜索到S库链接的所有动态库才会执行S库中的代码，即使S库所依赖的其他动态库都与S库处于同一目录，由于S库所在目录并不在以上五种搜索路径中，那么依然会失败。
